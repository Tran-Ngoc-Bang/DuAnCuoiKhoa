/**
 * Coin Packages Create Page JavaScript
 * Tạo gói xu mới với MVC thuần
 */
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function initializePage() {
        console.log('Initializing coin package create page...');
        try {
            setupFormValidation();
            setupPriceCalculation();
            setupCodeGeneration();
            console.log('Coin package create page initialized successfully');
        } catch (error) {
            console.error('Error initializing coin package create page:', error);
        }
    }
    ...
    function validateForm() {
        ...
        const requiredFields = ['code', 'name', 'coinAmount', 'originalPrice', 'salePrice', 'status'];
    ...
    function validateCode() {
        ...
        if (!/^[A-Z0-9-_]+$/.test(code)) {
            showFieldError(codeInput, 'Mã gói xu chỉ được chứa chữ hoa, số, dấu gạch ngang và gạch dưới');
            return false;
        }
