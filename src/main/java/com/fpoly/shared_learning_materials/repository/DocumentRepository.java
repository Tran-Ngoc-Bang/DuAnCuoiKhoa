...
public interface DocumentRepository extends JpaRepository<Document, Long>, JpaSpecificationExecutor<Document> {
        ...
        @Query("SELECT COUNT(d) FROM Document d WHERE d.file.uploadedBy = :user AND d.deletedAt IS NULL")
        ...
        @Query("SELECT COALESCE(SUM(d.viewsCount), 0) FROM Document d WHERE d.file.uploadedBy.id = :userId AND d.createdAt BETWEEN :from AND :to AND d.deletedAt IS NULL")
        int countViewsBetween(@Param("userId") Long userId,
                        @Param("from") LocalDateTime from,
                        @Param("to") LocalDateTime to);

        @Query("SELECT COALESCE(SUM(d.price), 0) FROM Document d WHERE d.file.uploadedBy.id = :userId AND d.createdAt BETWEEN :from AND :to AND d.deletedAt IS NULL")
        int sumCoinsBetween(@Param("userId") Long userId,
                        @Param("from") LocalDateTime from,
                        @Param("to") LocalDateTime to);
