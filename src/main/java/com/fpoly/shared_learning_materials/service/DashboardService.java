...
public class DashboardService {
    ...
    public List<Map<String, Object>> getLatestDocuments(int limit) {
        try {
            // Sử dụng query native SQL để lấy thông tin đầy đủ
            List<Map<String, Object>> documents = documentRepository.findLatestDocumentsWithDetails(limit);
            List<Map<String, Object>> result = new ArrayList<>();

            for (Map<String, Object> doc : documents) {
                Map<String, Object> docInfo = new HashMap<>();
                docInfo.put("id", doc.get("document_id"));
                docInfo.put("title", doc.get("document_title"));
                docInfo.put("status", doc.get("document_status"));
                docInfo.put("uploadDate", doc.get("upload_date"));
                docInfo.put("uploaderName", doc.get("uploader_name"));
                docInfo.put("categoryName",
                        doc.get("category_name") != null ? doc.get("category_name") : "Chưa phân loại");

                // Tính toán thời gian "time ago"
                if (doc.get("upload_date") != null) {
                    LocalDateTime uploadDate = (LocalDateTime) doc.get("upload_date");
            ...
            System.out.println("Found " + result.size() + " recent documents with details");
            return result;

        } catch (Exception e) {
            System.err.println("Error getting latest documents: " + e.getMessage());
            e.printStackTrace();
